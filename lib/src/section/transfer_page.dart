        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Tooltip(
                message: context.read<AppStatus>().isSyncing
                    ? "Automatic synchronization is enabled"
                    : "Receive data",
                child: Opacity(
                  opacity: context.read<AppStatus>().isSyncing ? 0.5 : 1,
                  child: IgnorePointer(
                    ignoring: context.read<AppStatus>().isSyncing,
                    child: btn(
                        icon: Icons.download,
                        context: context,
                        text: "Received",
                        ...
                        onpressed: () async {
                          if (context.read<AppStatus>().isSyncing) return;
                          setState(() {
                            isWaiting = true;
                          });
                          SocketServer.startServer(callback: () {
                            if (isWaiting) Navigator.of(context).pop();
                            setState(() {
                              isWaiting = false;
                            });
